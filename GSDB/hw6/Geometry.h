...
private:
    ...
    std::vector<LineString> interiorRingGroup;
...
public:
    ...
    Polygon(LineString& ering, std::vector<LineString>& iring) :exteriorRing(ering), interiorRingGroup(iring) { hasinterirorRing = true; hasinteriorGroup = true; constructEnvelope(); }
    virtual ~Polygon() {}

    LineString getExteriorRing() const { return exteriorRing; }
    LineString gerInteriorRing() const { return interiorRing; }
    std::vector<LineString> getInteriorRing() const { return interiorRingGroup; }
